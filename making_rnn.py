...
def loss_function(y_target,y,batch_size):
    loss=-np.sum(((y_target*np.log(y+0.0001))+(1-y_target*np.log(1-y+0.0001))),axis=0)
    loss/=batch_size
    return loss
...
class RNN_layer:
    ...
    def backward_RNN(self,y_target,y,x_data):
        ...
        da=-(np.divide(target,y_temp+0.00001)-np.divide(1-target,1-y_temp+0.00001))*y_temp*(1-y_temp)
